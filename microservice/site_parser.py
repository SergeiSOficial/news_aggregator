...
from configs.config import tralee_chat_id, kerry_chat_id
    ...
    while True:
        ...
        for row in lastnews:
            ...
            if send_message_func is None:
                ...
                await send_message_func(news_text,img_url, tralee_chat_id)
...
async def kerry_parser(httpx_client, posted_q, n_test_chars=50, 
    ...
    while True:
        try:
            ...
            if not (logger is None):
                logger.error(f'{source} error news headers\n{e}')
                print("error news headers")
        ...
        if len(lastnews) == 0:
            if not (logger is None):
                logger.error(f'{source} empty news')
                print("empty news")
        ...
        for row in lastnews:
            ...
            if send_message_func is None:
                ...
                await send_message_func(news_text,img_url, kerry_chat_id)
